...
void debugSymbolList() {
    tokenList *p = symPtr;
    printf("Debugging symbol list:\n");
    while (p != NULL) {
        printf("Node at %p: name='%s', line=%d\n", (void*)p, p->name ? p->name : "(null)", p->line);
        p = p->next;
    }
}
...
tokenList *getSymbol(char *token) {
    tokenList *p = symPtr;
    while (p != NULL) {
        printf("Checking symbol: %s at %p\n", p->name, p);
        if (p->name != NULL && strcmp(token, p->name) == 0) {
            printf("Found symbol '%s' at %p with line %d\n", p->name, p, p->line);
            return p;
        }
        p = p->next;
    }
