...
program_head
    : program
    {
        ...
        if (mainFunc == NULL) {
            fprintf(stderr, "ERROR at line %d: Main function 'main' not found!\n", yylineno);
            yyerror();
            YYABORT;
        }
	printf("mainFunc = %p, mainFunc->line = %d\n", (void*)mainFunc, mainFunc ? mainFunc->line : -1);
        backpatch($1.nextList, mainFunc->line + 1);
        $$ = $1; // Optional: propagate the nextList if needed by parent rules
    }
;
